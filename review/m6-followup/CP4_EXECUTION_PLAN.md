...
4. Implement a resumable one-shot activator. Revalidate the protected journal
   before each idempotent substep; rollback definition and database together on
   pre-COMMIT failure, and enter forward-only recovery after durable commit
   authorization. The activator consumes authorization and never invents a
   controller-wide CP5 COMMIT decision.
5. Extend the existing protected local bridge and CLI with prepare/resume,
   ...
   upgrade methods.
6. Test journal crash points, database WAL/shadow/switch/rollback, all platform
