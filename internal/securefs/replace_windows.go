...
func replace(root *os.Root, temporary, destination string) (bool, error) {
	...
	attributes := &windows.OBJECT_ATTRIBUTES{
		Length: uint32(unsafe.Sizeof(windows.OBJECT_ATTRIBUTES{})), RootDirectory: windows.Handle(directory.Fd()),
		ObjectName: objectName, Attributes: windows.OBJ_CASE_INSENSITIVE | windows.OBJ_DONT_REPARSE,
	}
	var handle windows.Handle
	err = windows.NtCreateFile(
		&handle, windows.FILE_GENERIC_WRITE|windows.DELETE|windows.SYNCHRONIZE, attributes,
	...
	err = windows.NtSetInformationFile(
		handle, &windows.IO_STATUS_BLOCK{}, &buffer[0], uint32(len(buffer)), windows.FileRenameInformation,
	)
	if err != nil {
		return false, windowsError(err)
	}
