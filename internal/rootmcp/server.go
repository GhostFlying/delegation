...
const (
	...
	serverInstructions = "Delegation exposes live peers, synchronized Git workspaces, and managed agents. Use list_devices, then describe_device before choosing an online peer; isCurrentDevice allows self-targeting. For repository work, call sync_workspace with a fresh sync_id, an explicit Git URL, and the selected target; then pass the returned workspace_id to spawn_agent. remote_git_full_history_fallback means the transfer includes HEAD-reachable history that may contain deleted content; disclose that warning to the user. Call spawn_agent with a fresh spawn_id and self-contained task. busy means no target slot; indeterminate means no definitive target result was confirmed or recorded and the worker may have started. Retry either with the same ID and exact arguments. Use list_agents for durable receipts and wait_agent for lifecycle activity, worker messages, or changes artifact metadata; call wait_agent again while has_more is true. In artifacts, base_warnings and result_warnings are independent. Target payloads are best-effort, up to 64 within 2 GiB; broker metadata may outlive them and does not prove availability. This version cannot download, apply, or write payloads back. Targets accept an agent UUID, task_name, or /root/task_name. Use a fresh message_id or operation_id for each logical action; retry uncertain results with the same ID and arguments. send_message delivers to running or idle agents, followup_task starts idle agents, and interrupt_agent stops active turns."
)
...
func NewServer(backend Backend, controllerID, deviceID string) (*mcp.Server, error) {
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Wait for managed-agent lifecycle activity, worker messages, and changes-artifact metadata in this root task.",
