...
// authenticated source. Delivered records remain visible so a lost final
// acknowledgement can be replayed without transferring bytes again.
...
func scanResultPackage(scanner rowScanner) (ResultPackageRecord, error) {
	...
	case ResultPackageDeliveryPending:
		if record.Sequence != 0 || record.DeliveredAt != 0 {
